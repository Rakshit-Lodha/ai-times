...
export default function StoryCard({ article, isPriority = false, onUndo, canUndo = false, variant = "news", positionLabel }: StoryCardProps) {
    ...
    <>
      <article ref={articleRef} className="h-full min-h-full w-full overflow-y-auto overflow-x-hidden bg-[#080808] no-scrollbar md:min-h-0 md:h-full">
        ...
        <div className="relative aspect-[3/2] w-full overflow-hidden">
          ...
          {/* Top bar - date/position and share */}
          <div className="absolute right-4 top-4 flex items-center gap-2">
            {/* Date pill (news) or position pill (learn) */}
            <div className="rounded-full bg-black/40 px-3 py-2 backdrop-blur-md">
              <span className="text-[12px] font-medium text-white/70">
                {isLearn && positionLabel ? positionLabel : formatDate(article.created_at || article.news_date || "")}
              </span>
            </div>
            {/* Share button */}
        ...
        <div className="mx-auto max-w-[540px] px-7 pb-10 pt-5 sm:px-8">
          ...
          {/* Action buttons row (news only) */}
          {!isLearn && (
            <div className="mt-6 flex items-center gap-3">
              {/* Go Deeper button */}
              <button
                onClick={() => setIsDeepDiveSheetOpen(true)}
                className="flex items-center gap-2 rounded-full border border-white/20 bg-white/5 px-4 py-2.5 transition-all hover:bg-white/10 active:scale-95"
              >
              ...
              {/* WhatsApp daily digest button */}
              <a
                href="https://chat.whatsapp.com/LjKtv0EcXvF3pohJteAqn7?mode=gi_t"
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-2 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-4 py-2.5 transition-all hover:bg-emerald-500/20 active:scale-95"
              >
                ...
                <span className="text-[0.9rem] font-medium text-emerald-400/90">Daily Digest</span>
              </a>
            </div>
          )}
        </div>
